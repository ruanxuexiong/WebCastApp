...
public abstract class BaseListViewFragment<TViewHolder, TBaseJsonAdapter extends BaseJsonAdapter<TViewHolder>> extends BaseRequestFragment implements PullToRefreshLayout.OnRefreshListener {
	...
	public void locationData() {
		...
		mTxtLoad.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				UIHelper.showOnLoadingDialog(getActivity(), "正在努力加载中");
	...
	protected IOAuthCallBack mIOAuthCallBack = new IOAuthCallBack() {
		...
		public void getSuccess(String successJson) {
			...
			if(mResult.mIsSuccess) {
				...
				if (mPageIndex == mBaseApplication.getFirstPageIndex()){
					mVgLoad.setVisibility(View.VISIBLE);
					mTxtText.setText(R.string.default_load_null_str); // default_load_fail_str
					if (!mPullToRefreshType) {
						mPullToRefreshFinish.setVisibility(View.GONE);
					} else {
						mPullToLoadmore.setVisibility(View.GONE);
	 ...
	 * 开始请求数据
	 */
	protected void onStartListRequest(){
	 ...
	 * 数据请求完成
	 */
	protected void onCompletedList(String successJson){
