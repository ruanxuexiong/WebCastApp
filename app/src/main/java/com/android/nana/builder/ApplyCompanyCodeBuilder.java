...
public class ApplyCompanyCodeBuilder implements CountDownBuilder.OnCountRunning {
	...
	public ApplyCompanyCodeBuilder(Context context, EditText editPhone, TextView validateCode) {
		mContext = context;
	...
	public ApplyCompanyCodeBuilder(Context context, String phone, TextView validateCode) {
		mContext = context;
		mPhoneNumber = phone;
		mTxtValidateCode = validateCode;
		mCountDownBuilder = new CountDownBuilder(this);
	...
	public void getCardByPhone(String userId,String phone,String time,String sign, final PhoneValidateCodeListener validateCodeListener){
		
		if (!mIsAllowed) return;

		RecruitDbHelper.sendCompanyCode(userId, phone, time, sign, new IOAuthCallBack() {
			@Override
