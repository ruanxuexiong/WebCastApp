...
public class SealAppContext implements RongIM.ConversationListBehaviorListener,
    ...
    private void initListener() {
        ...
        RongIM.setConversationListBehaviorListener(this);//会话列表监听事件
    ...
    public boolean onConversationClick(final Context context, View view, final UIConversation uiConversation) {
        ...
        if (uiConversation.getConversationType().getName().equals("private")) {
            String mid = (String) SharedPreferencesUtils.getParameter(context, "userId", "");
            HomeDbHelper.isFriend(mid, uiConversation.getConversationTargetId(), new IOAuthCallBack() {
                @Override
                    ...
                    try {
                        ...
                        if (result.getString("state").equals("0")) {
                            if (data.getString("friend").equals("2")) {//非好友状态
                                ...
                                RongIM.getInstance().startPrivateChat(context, uiConversation.getConversationTargetId() + "=" + isTanchang + "=" + mMoney + "=" + mEnough + "=", uiConversation.getUIConversationTitle());
                                return;
                            } else {
            ...
            RongIM.getInstance().startGroupChat(context, "group_"+uiConversation.getConversationTargetId(), uiConversation.getUIConversationTitle());
            return true;
        }
        return false;
    }
    ...
    public boolean onReceived(final Message message, int i) {
        ...
        if (message.getTargetId().contains("zx_")) {
            if (MainListener.getInstance().mOnMessageRefreshListener != null) {
                MainListener.getInstance().mOnMessageListener.refersh(message);
            }
    ...
    public void onStartLocation(Context context, LocationCallback locationCallback) {
         ...
         */
        Toast.makeText(context, "地图地图哦", Toast.LENGTH_SHORT).show();
    }
    ...
    private void logout(final boolean isKicked) {
        String mui = (String) SharedPreferencesUtils.getParameter(getContext(), "userId", "");
            ...
            public void getSuccess(String successJson) {
                try {
                    ...
                    if (jsonObject1.getString("state").equals("0")) {
                        RongIM.getInstance().logout();
                        SharedPreferencesUtils.setParameter(getContext(), "userId", "");
                        SharedPreferencesUtils.saveObject(getContext(), "userInfo", null);
                        SharedPreferencesUtils.setParameter(getContext(), "mtcLogouted", "mtcLogouted");
                        deleteData();//清空搜索历史
                        Intent loginActivityIntent = new Intent();
                        loginActivityIntent.setClass(mContext, com.android.nana.auth.WelcomeActivity.class);
    ...
    @Override
    public void getGroupMembers(String groupId, final RongIM.IGroupMemberCallback callback) {//群组@功能


        HomeDbHelper.memberList(groupId, "", new IOAuthCallBack() {
            @Override
            public void onStartRequest() {
            }
                    ...
                    if (result.getString("state").equals("0")) {
                        ...
                        if (mFriendsData.size() > 0) {
                            for (FriendsBookEntity entity : mFriendsData) {
                                UserInfo userInfo = new UserInfo(entity.getId(), entity.getUsername(), Uri.parse(entity.getAvatar()));
