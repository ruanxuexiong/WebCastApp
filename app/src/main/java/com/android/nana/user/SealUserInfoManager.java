    ...
    public void getUserInfo(final String userId) {
        ...
        if (mUserInfoCache != null) {
            ...
            if (userInfo != null) {
                ...
                Log.d(TAG, "SealUserInfoManager getUserInfo from cache " + userId + " "
        ...
        mWorkHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (friend != null) {
                    ...
                    Log.d(TAG, "SealUserInfoManager getUserInfo from Friend db " + userId + " "
                            + userInfo.getName() + " " + userInfo.getPortraitUri());
                    RongIM.getInstance().refreshUserInfoCache(userInfo);
                    return;
                }
                ...
                if (groupMemberList != null && groupMemberList.size() > 0) {
                    ...
                    Log.d(TAG, "SealUserInfoManager getUserInfo from GroupMember db " + userId + " "
                            + userInfo.getName() + " " + userInfo.getPortraitUri());
                    RongIM.getInstance().refreshUserInfoCache(userInfo);
                    return;
                }
      ...
      /*  mWorkHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (group != null) {
                    ...
                    Log.d(TAG, "SealUserInfoManager getGroupInfo from db " + groupsId + " "
        ...
        UserInfoEngine.getInstance(mContext).setListener(new UserInfoEngine.UserInfoListener() {
            ...
            public void onResult(UserInfo info) {
                if (info != null && RongIM.getInstance() != null) {
                    ...
                    Log.d(TAG, "SealUserInfoManager getUserInfo from network " + info.getUserId() + " " + info.getName() + " " + info.getPortraitUri());
 ...
 /*   public void getAllUserInfo() {
    ...
    private boolean fetchFriends() throws HttpException {
        ...
        try {
            ...
            Log.d(TAG, "fetchFriends occurs JSONException e=" + e.toString());
            return true;
        }
    ...
    private List<Friend> pullFriends() throws HttpException {
        ...
        try {
            ...
            Log.d(TAG, "pullFriends occurs JSONException e=" + e.toString());
            return null;
        }
        if (userRelationshipResponse != null && userRelationshipResponse.getCode() == 200) {
            List<UserRelationshipResponse.ResultEntity> list = userRelationshipResponse.getResult();
        ...
        if (mGroupsList != null && mGroupsList.size() > 0) {
            ...
            for (Groups group : mGroupsList) {
                ...
                try {
                    try {
                        groupMemberResponse = action.getGroupMember(group.getGroupsId());
                    } catch (com.android.webcast.network.http.HttpException e) {
                        e.printStackTrace();
                    }
                } catch (JSONException e) {
...
*/
 ...
 /*   private List<Groups> addGroups(final List<GetGroupResponse.ResultEntity> list) {
  ...
  public List<BlackList> addBlackList(List<GetBlackListResponse.ResultEntity> responseList) {
            ...
            public void run() {
                ...
                if (!doingGetAllUserInfo && !hasGetFriends()) {
                    ...
                    try {
                        ...
                        onCallBackFail(callback);
                        Log.d(TAG, "getFriends occurs HttpException e=" + e.toString() + "mGetAllUserInfoState=" + mGetAllUserInfoState);
                        return;
                    }
                } else {
     ...
     */
    public List<Groups> getGroups(ResultCallback<List<Groups>> resultCallback) {
        if (mGroupsDao != null) {
