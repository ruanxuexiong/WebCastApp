...
public class LabelsView extends ViewGroup implements View.OnClickListener {
    ...
    public void setLabelsSelect(ArrayList<String> labels) {
        //清空原有的标签
        clearAllSelect();
        removeAllViews();
        mLabels.clear();
        ...
        if (labels != null) {
            ...
            int size = labels.size();
            for (int i = 0; i < size; i++) {
                addLabelSelect(labels.get(i), i);
            }
        }
    }

    private void addLabelSelect(String text, int position) {

//        Log.e("返回值大家积极===", text + "====" + position);
        ...
        label.setText(text);
        if (mLabelBgResId != 0) {
            label.setBackgroundResource(R.color.main_blue);
        }
    ...
    //取消单个选中
    public void clearSingleSelect(int i) {
        setLabelSelect((TextView) getChildAt(i), false);
    }

    //选中
    public void setSingleSelect(int i) {
        setLabelSelect((TextView) getChildAt(i), true);
    }
