...
public class FollowCustomRecyclerView extends RecyclerView {
    ...
    public FollowCustomRecyclerView(Context context) {
        super(context);
    }

    public FollowCustomRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);


    }

    public FollowCustomRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
...
//                        Log.d("pos", "recyclerview left: " + rect_parent.left + " | right: " + rect_parent.right + " | top: " + rect_parent.top + " | bottom: " + rect_parent.bottom);
                if (playOnlyFirstVideo) {
                    ...
                    for (int i = firstVisiblePosition; i <= lastVisiblePosition; i++) {
                        ...
                        try {
                            FollowCustomViewHolder cvh = (FollowCustomViewHolder) holder;
...
//                                    Log.d("trace", i + " foundFirstVideo: " + cvh.getVideoUrl());
                                    ...
                                    /*if (getString(_act, cvh.getVideoUrl()) != null && new File(getString(_act, cvh.getVideoUrl())).exists()) {
                                        ((NearbyCustomViewHolder) holder).initVideoView(getString(_act, cvh.getVideoUrl()), _act);
                                    } else {*/
                                    ((FollowCustomViewHolder) holder).initVideoView(cvh.getVideoUrl(), _act);
                                    // }
                                    if (downloadVideos) {
                                    ...
                                    Runnable myRunnable = new Runnable() {
                                        @Override
                                        public void run() {
                                            if (!((FollowCustomViewHolder) holder).isPaused())
                                                ((FollowCustomViewHolder) holder).playVideo();
                                        }
                                    };
...
//                                    Log.d("trace", i + " not foundFirstVideo: ");
                                    ((FollowCustomViewHolder) holder).pauseVideo();
                                }
                            }
                        } catch (Exception e) {
                        }
...
//                                Log.d("trace", i + " contains: " + rect_parent.contains(rect_child));
                                ...
                                if (percent >= visiblePercent) {
                                    ...
                                    ((NearbyCustomViewHolder) holder).initVideoView(cvh.getVideoUrl(), _act);
                                    //}
                                    if (downloadVideos) {
                                        startDownloadInBackground(cvh.getVideoUrl());
                                    }
                                    Runnable myRunnable = new Runnable() {
                                        @Override
                                        public void run() {
                                            if (!((NearbyCustomViewHolder) holder).isPaused())
                                                ((NearbyCustomViewHolder) holder).playVideo();
                                        }
                                    };
                                    handler.post(myRunnable);
                                    runnables.add(myRunnable);
                                } else {
                                    ((NearbyCustomViewHolder) holder).pauseVideo();
                                }
                            }
                        } catch (Exception e) {
    ...
    public void stopVideos() {
       ...
       for (int i = 0; i < getChildCount(); i++) {
            if (findViewHolderForAdapterPosition(i) instanceof NearbyCustomViewHolder) {
                final NearbyCustomViewHolder cvh = (NearbyCustomViewHolder) findViewHolderForAdapterPosition(i);
